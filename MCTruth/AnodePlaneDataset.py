# Class that allows a directory containing .safetensor files generated by CreateMCNNData.py to be read in by PyTorch via DataSet/DataLoader classes
...
# Making sure that the correct tensor are present in the file
                    ...
                    if input_name not in key_names:
                        raise Exception(input_name+" not found")
                    if output_name not in key_names:
